...
@<br>{}
メソッド：PUT@<br>{}
URL：http://(Hue BridgeのIPアドレス)/api/newdeveloper/groups/0/action@<br>{}
出力形式：JSON

IPアドレスは2.6で"internalipaddress"として返されたIPアドレスを使用します。

newdeveloper の部分は Hue のユーザ名を使用してください。(デフォルトでは newdeveloper)

以上の設定が終わったら、プロパティ画面を閉じて、右上にある「デプロイ」を選択します。その後、「ライト」ノードの下に小さく「 disconnected 」と表示されますが、しばらく待つと「 connected 」に変わります。この状態になっていなければ、Alexa と接続状態ではないので、Alexa アプリ、ブリッジ、Node-RED の間で何らかの設定を間違えている可能性があります。
...
ちなみに、ここでは Hue に点灯のための最低限のメッセージしか送っていませんので、点灯する色や明るさは最後の点灯の際の状態と同じになります。ここを変更したければ、「 template 」ノードの JSON を Hue の仕様に従って変える必要があります。うちの場合は～と設定しています。
