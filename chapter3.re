...
== 3.1 シーリングライトとHueを連動させる（IRKitを使う）
いくらHueが便利でも、家の照明をすべてHueにさせている人は珍しいと思います。Hue単体では部屋が暗くなってしまうので、Hueと一緒に既存のシーリングライトを使用されている方が多いかもしれません。ただ、Hueとシーリングライトを別々に操作するのは煩雑で、何のためにバカ高い電球を導入したのか分からなくなっちゃいますよね。そこで、Hueを点けたときはシーリングライトを消し、シーリングライトを点けたときはHueを消すような連動制御をしたいと思います。

用意するもの

IRKit
（※本書ではIRKitを説明に使用していますが、同様にAPIを持つiRemoconでも同じことが可能です）
...
== 3.3 「バルス」でRaspberry Piの電源を落とす（スピーカーとの接続）
...
4.1ではcurlコマンドを用いてIRKitのAPIを操作しましたが、このcurlコマンドをshutdownコマンドに差し替えればやりたいことができます。ただ、Raspberry Piの電源がオフになったかどうかはやや分かりにくいところがあるので、本当に命令が効いているのか、何らかのリアクションが欲しいですよね。というわけでついでにスピーカーにも接続して、電源オフの命令を受信後にまずリアクション音を鳴らしてからシャットダウン処理に入るようにしましょう。音声コマンドは「バルス」とします。
...
4.2では毎日決まった時間にシーリングライトからHueへと照明を切り替えるようにしました。しかし毎日この時間に人がいるとは限りません。外出中にこれを動かすと、消えていたはずの照明を付けることになり、とてもスマートとは言えません。
...
・シェルスクリプトを作成する
・リアクション音をつける
== 3.5 帰宅時に電気を付ける（Bluetoothポーリング機能）
4.4では外出中フラグを管理することで在宅か不在かをシステムに認識させるようにしました。このトリガーは外出時も帰宅時も手動で行う必要がありますが、自動で検知してくれると素敵ですよね。
...
これで帰宅時の自動検知ができるようになりました。外出時も同様に自動検知したいのですが、我が家では以下の理由から外出時は手動でトリガーを引く方が確実であると結論づけているので、完全自動化は皆さんの知恵にお任せすることにします。

・外出時に照明・エアコンが確実にオフになっていることを目視で確認したい
（不在の状態で誤作動することを避け、常に安全側に倒したい）
...
== 3.9 複数家電のマクロリモコンをつくる（外部シェルへの切り出し・IRKitを使う）
IRKitのAPIを使ったリモコンの制御
仮想的にTVを実現する
エアコンとサーキュレータを同時にオンオフする
== 3.10 Alexaの代わりにミクさんにおしゃべりさせる（OpenJTalkを使う）
OpenJTalkの導入
== 3.11 音声を使わずにAlexaを操作する（OpenJTalkの応用）
スケジュール機能のないAlexaをOpenJTalkを使って操作する
== 3.12 もっと流暢に喋らせる（フリー音源の活用）
フリー音源サイトを紹介するだけ
例も少し入れる
== 3.13 定時に外気温・室温・湿度を声でお知らせする（netatmoのデータ取得）
OpenJTalkの応用編。高いけどnetatmoを使う。天気予報サイトを使っても可
== 3.14 日の出・日の入りに鐘の音を鳴らす（day-night.sh）
カーテンの開け閉めタイミングを音でお知らせ。応用するとカーテン開け閉め含めて自動化できる
== 3.15 夜7時になるとNHKニュースを流す（cvlcによるストリーミング再生）
お気に入りのラジオ番組を自動的に流して終了させる仕組み
== 3.16 リモコンのボタン1つで特定のインターネットラジオを再生する・終了する（cvlcによるストリーミング再生・応用編）
